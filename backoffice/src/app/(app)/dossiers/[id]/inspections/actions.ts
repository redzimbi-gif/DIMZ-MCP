...
export async function createInspection(dossierId: string, formData: FormData) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Erreur inconnue lors de la création de l'inspection.";
    redirect(`/dossiers/${dossierId}/inspections/new?error=${encodeURIComponent(message)}`);
  }

  revalidatePath(`/dossiers/${dossierId}`);
  revalidatePath("/inspections");
  redirect(`/dossiers/${dossierId}/inspections/${newId}`);
}
